...
def main_on_back(hold = 3, accel = 1, num_times = 1, interval = 3):
    Move = MovementGroups()
    
    for _ in range(num_times):
        Move.foreleg_lift("left", 0.05, hold, accel)
...
def main_walk(direction="", num_times = 1):
    ...
    if (direction == "left"):
        ...
        Move.move_backward()
    elif (direction == "onback"):
        for _ in range(num_times):
            Move.gait_uni(0.5, 0, 2, 1)
...
def yoga():
    # main ground stretching
    time.sleep(5)
    main_cat_cow(3,4)
    time.sleep(2)
    main_leg_lifts(3, 1, 3, 1)
